...
def insert_student(name, roll_no, course):
    ...
    try:
        ...
        # Check duplicates manually before inserting
        ...
        # Only insert user if username doesn't already exist
...
def get_pass_fail_stats(pass_marks=40):
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT s.id, s.name, COUNT(m.id) AS subject_count,
        ...
        for _, _, subject_count, passed_subjects in rows:
